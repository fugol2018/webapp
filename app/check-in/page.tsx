...
export default function CheckInSelectPage() {
  ...
  const canCheckIn = (vehicle: Vehicle) => {
    return vehicle.registrationCompleted && vehicle.status === 'checked_out';
  };
      ...
      <div className="header-dark-gradient text-white sticky top-0 z-30 overflow-hidden">
        ...
        <div className="relative z-10 px-4 py-4 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold">Check-in Vehicle</h1>
              <p className="text-sm text-white/50">Select a vehicle to check in</p>
            </div>
          </div>
        ...
        {vehicles.length === 0 ? (
          ...
          <>
            <div className="px-1 py-2">
              <p className="text-sm text-muted-foreground">
                <span className="font-semibold text-foreground">{vehicles.length}</span> vehicle{vehicles.length !== 1 ? 's' : ''} available for check-in
              </p>
            </div>

            {vehicles.map((vehicle, idx) => {
              const client = getClient(vehicle.clientId);
              const checkInAllowed = canCheckIn(vehicle);
              ...
              return (
                ...
                >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {!checkInAllowed && (
                        <div className="mt-2 flex items-start gap-2 p-2.5 bg-destructive/[0.06] border border-destructive/10 rounded-lg text-xs text-destructive">
                          ...
                          <span>Registration incomplete. Complete required steps to check in.</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="mt-4">
                    {checkInAllowed ? (
                      <Link href={`/vehicles/check-in/${vehicle.id}`} className="block">
